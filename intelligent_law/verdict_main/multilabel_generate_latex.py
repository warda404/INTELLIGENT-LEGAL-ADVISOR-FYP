...
import numpy as np
import copy
from my_config import MULTILABEL_OUTPUT_FILE
from format import  \
    data_to_article, \
    FLAVORS_SHORT_FORM
...
def generate_latex_table_multilabel_article(name, _data, key="acc", std=True, order=max):
    ...
    for method, datasets in _data.items():
        for dataset, res in datasets.items():
            if dataset not in best_per_dataset:
                best_per_dataset[dataset] = np.round_(
                    res['test']['test_{}'.format(key)], 4)
            else:
                best_per_dataset[dataset] = order(best_per_dataset[dataset], np.round_(
                    res['test']['test_{}'.format(key)], 4))
    ...
    for i, method in enumerate(sorted(data.keys())):
        latex_output += '{message:<{fill}}'.format(
            message=method, fill=max_m)
        for dataset in dataset_short.keys():
            if dataset in data[method]:
                ...
                if std:
                    latex_output += ' ({:.2f})'.format(
                        np.round_(d['test']['test_{}_std'.format(key)], 2))
            else:
...
def main():
    ...
    keys = ['acc', 'precision', 'recall', 'f1_weighted', 'zero_one_loss',
            'jaccard_similarity_score', 'hamming_loss']  # , 'balanced_acc']
