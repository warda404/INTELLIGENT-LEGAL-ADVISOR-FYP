...
def calculate_average_cm(CM, train_score=True):
    rounder = np.vectorize(
        lambda x: '{:.4f}'.format(np.round_(x, ROUND_DIGITS)))
    result = {}
    ...
    if train_score:
        ...
        result['test_n'] = rounder(avg_cm.astype(
            'float') / avg_cm.sum(axis=1)[:, np.newaxis]).tolist()
        result['train'] = np.sum([i / float(folds)
                                  for i in CM[1::2]], 0)
        result['train_n'] = rounder(result['train'].astype(
            'float') / result['train'].sum(axis=1)[:, np.newaxis]).tolist()
